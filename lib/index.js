...
function proxy(req, options, res) {
  ...
  if (!ALLOWED_PROTOCOLS.includes(options.protocol)) {
    throw new ProxyRequestError('protocol not supported');
  }
  ...
  const agentReq = req.pipe(reqCacheStream).pipe(transport.request(options));
  ...
  return new Promise((resolve, reject) => {
    ...
    agentReq.once('response', (agentRes) => {
      ...
      if (isRedirecting(agentRes)
        ...
        const redirectReq = reqCacheStream.pipe(transport.request(newOptions));
